...
async function main() {
  ...
  for (const address of TARGET_ADDRESSES) {
    ...
    try {
      ...
      if (history && history.length > 0) {
        const signatures = history.slice(0, 3).map(tx => tx.signature);
  ...
  results.forEach(result => {
    if (result.status === 'SUCCESS' && result.recentSignatures) {
      ...
      result.recentSignatures.forEach((sig, i) => {
        console.log(`  ${i + 1}. ${sig}`);
  ...
  if (readySignatures.length > 0) {
    ...
    console.log('\nSAMPLE REANNOUNCE SIGNATURES:');
    readySignatures.slice(0, 10).forEach((item, i) => {
      console.log(`${i + 1}. ${item.signature} (${item.address.slice(0, 8)}...)`);
    });
  }
